...
function add_device($extname,$extuser,$extpass) {

//$gen_creds = "echo -n" . $arguments['u'] . ':' . $arguments['p'] . '  ' . '|' . 'md5sum | cut -d" " -f1'; 
//$creds = shell_exec(rtrim($gen_creds));
//echo $creds;
    
$var_dev = '{ "data": {"name":' . '   ' . '"' . $extname . '"' . ',' .  '   ' . '"' . "sip.username" . '"'    . ':' . '  ' . '"' . $extuser  .  '"'. ',' . ' ' . '"' .  "sip.password". '"' . ":" . '  ' . '"' . $extpass . '"' . '}' . '}';
...
$user_account = 'http://' . $arguments['i'] . ':8000/v2/accounts/' . $acc_id . '/devices';   
...
for ($a =0; $a < $cnt ; $a++) {
 ...
 exec('curl  -X PUT -H "Content-Type: application/json" -H "X-Auth-Token:' . ' '. $token . '"' . ' ' . '-d ' . "'" .  add_device($csvarr[$a][0],$csvarr[$a][1],$csvarr[$a][2]) . "'" . '  ' . $user_account . '|' . 'python -mjson.tool',$outfor);
// echo 'curl  -X PUT -H "Content-Type: application/json" -H "X-Auth-Token:' . ' '. $token . '"' . ' ' . '-d ' . "'" .  add_device($csvarr[$a][0],$csvarr[$a][1],$csvarr[$a][2]) . "'" . '  ' . $user_account . '|' . 'python -mjson.tool';
 print_r($outfor);
}